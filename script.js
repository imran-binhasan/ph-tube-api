...
const loadVideos = async (keyword = "") => {
  const res = await fetch(
    `https://openapi.programming-hero.com/api/phero-tube/videos?title=${keyword}`
  );
  const data = await res.json();
...
const displayVideos = (videos) => {
  ...
  if (videos.length === 0) {
    document.getElementById("video-container").classList =
      "w-11/12 mx-auto gap-6 py-10";
    document.getElementById(
      "video-container"
    ).innerHTML = `<div class="flex items-center justify-center flex-col">
                    <div class="min-h-vw">
    ...
    document.getElementById("video-container").classList =
      "w-11/12 mx-auto  grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 py-10";
  }
    ...
    div.innerHTML = `
        ...
        ${
          element.authors[0].verified === true
            ? `<img class="h-[20px]" src="https://img.icons8.com/?size=48&id=D9RtvkuOe31p&format=png">`
            : ""
        }
        </div>
        <p class="text-gray-600">${element.others.views} Views</p>
        <button class="btn bg-pink-600 text-white btn-sm" onclick="handleModal('${
          element.video_id
        }')">Details</button>
        </div>
        </div>`;
...
// Remove Search Input on Blur
document.getElementById("input-search").addEventListener("blur", (e) => {
